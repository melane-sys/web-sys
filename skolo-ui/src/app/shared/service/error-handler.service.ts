...
export class ErrorHandlerService implements HttpInterceptor {
  ...
  public handleForbidden = (error: HttpErrorResponse) => {
    this.router.navigate(["/forbidden"], { queryParams: { returnUrl: this.router.url }});
    
    return "Forbidden";
  }

  public handleNotFound = (error: HttpErrorResponse): string => {
    this.router.navigate(['/404']);
    return error.message;
  }
  
  public handleUnauthorized = (error: HttpErrorResponse) => {
  ...
  public handleBadRequest = (error: HttpErrorResponse): string => {
